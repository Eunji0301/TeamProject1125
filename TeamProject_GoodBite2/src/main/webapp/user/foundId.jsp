<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>GoodBite Found Id</title>
<link rel="stylesheet" type="text/css" href="../resources/css/bootstrap-5.3.3-dist/css/bootstrap.css">
<link rel="stylesheet" type="text/css" href="../resources/css/bootstrap-5.3.3-dist/css/bootstrap.min.css">
<link rel="stylesheet" type="text/css" href="../resources/css/sycss/user.css">

</head>
<body>
    <main id="found">
        <div class="foundInner">
            <h1 class="title">아이디 찾기 결과</h1>
            <div class="foundBorder p-4">
            	<div class="text-center my-3">
            		<div class=" foundSubTitle">고객님의 아이디 찾기가<br>완료되었습니다.</div>
            		<hr>
            	</div>
            	
	            <div class="foundInBorder p-2 p-4">
		            <div class="foundInfo ps-1" >
		            	<div><strong>이 름 : </strong><span>홍길동</span></div>
		            	<div><strong>이메일 : </strong><span>test1@gmail.com</span></div>
		            </div>
	            	<hr>
	            	 <div class=" ps-1  d-flex align-items-center">
	            	 	<div class="foundCircle me-1"></div><strong>test1(아이디)</strong><span>[2024-12-14(가입)]</span></div>
	            	</div>
                <div class=" d-flex mt-3">
	                <button type="button" class="loginButton me-1">로그인</button>
	                <button type="button" class="searchPwdButton ms-1">비밀번호 찾기</button>
                </div>
            </div>
        </div>
    </main>

</body>
</html>